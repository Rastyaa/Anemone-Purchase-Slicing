...
4. **Struktur katalog.** Figma menampilkan katalog sebagai list satu kolom (tiap produk full-width). Frontend menyusunnya ulang jadi grid multi-kolom (`sm:grid-cols-2 xl:grid-cols-3`) — lebih efisien memanfaatkan ruang layar lebar, dan lebih scalable untuk katalog HO yang realistiknya jauh lebih besar dari 6 produk mock ini.
...
- **Section "Butuh Bantuan?"** (kontak Head Office) yang ada di desain Figma **belum diimplementasikan** di frontend ini — bukan keputusan sadar untuk dihapus, tapi belum sempat ditambahkan dalam batas waktu pengerjaan. Ini known gap, bukan penyimpangan desain yang disengaja seperti poin-poin di §Kenapa Tampilan Frontend Tidak 1:1 dengan Figma.
